...
func where_first():
	...
	DL.prog_text()
	await DL.advance_text
	DL.swap_dialogue("I know. But there was no investigation at all.")
	DL.prog_text()
	await DL.advance_text
	DL.swap_dialogue("We need to at least attempt one, like old times! Before I left for the church.")
	DL.prog_text()
	await DL.advance_text
	DL.swap_name("Roman")
